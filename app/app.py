...
# ============================
# Carga, predicción y evaluación (robustos a distintos .joblib)
# ============================

@st.cache_resource(show_spinner=False)
def load_model_safely(model_path: str):
    """Carga con caché (Streamlit) y manejo de errores."""
    try:
        return joblib.load(model_path)
    except Exception as e:
        st.warning(f"No se pudo cargar el modelo: {model_path}. Detalle: {e}")
        return None


def _get_classes_from_estimator(est):
    """Devuelve el vector de clases del estimador si existe."""
    return getattr(est, "classes_", None)


def _proba_or_score_to_p1(estimator, X):
    """
    Devuelve vector de probabilidades [p0, p1, ...] si es posible.
    - Si hay predict_proba => lo usamos.
    - Si no, usamos sigmoide(decision_function) (binario) => [1-p, p].
    - Último fallback: .predict como 0/1 => [1-pred, pred].
    """
    if hasattr(estimator, "predict_proba"):
        proba = estimator.predict_proba(X)[0]  # (n_clases,)
        return proba
    elif hasattr(estimator, "decision_function"):
        import math
        score = float(estimator.decision_function(X)[0])
...
def predict_label_from_model(model_obj, text: str):
    """
    Devuelve un dict con la predicción usando la **regla y threshold del propio modelo**:
    {
      'pred': 0/1,
      'p1': prob de la clase target_class (si aplica),
      'threshold': usado,
      'target_class': usado,
      'classes': orden de clases del estimador final (si existe)
    }

    Soporta tres formatos:
      a) dict {'vectorizer','clf','threshold',...}
      b) dict {'pipeline','threshold','target_class','decision',...}
      c) estimador/pipeline sklearn directo con .predict
    """
    Xtxt = [text]

    # ---- (b) Diccionario avanzado: pipeline + metadata ----
    if isinstance(model_obj, dict) and "pipeline" in model_obj:
        pipe = model_obj["pipeline"]
        threshold = float(model_obj.get("threshold", 0.5))
        target_class = int(model_obj.get("target_class", 1))
        clf = pipe.named_steps.get("clf", pipe)  # estimador final si está en named_steps
        classes = _get_classes_from_estimator(clf)

        # Probabilidades / scores
        if hasattr(pipe, "predict_proba"):
            pvec = pipe.predict_proba(Xtxt)[0]
        else:
            # transformar hasta el clf
            try:
                X_trans = pipe[:-1].transform(Xtxt)
            except Exception:
                # si no se puede cortar el pipeline, intentamos transform directamente
                X_trans = Xtxt
            pvec = _proba_or_score_to_p1(clf, X_trans)

        # Índice de la clase objetivo
        if classes is not None:
            idx = int(np.where(classes == target_class)[0][0])
        else:
            # asumimos binario [p0, p1]
            idx = target_class

        p1 = float(pvec[idx])
        pred = target_class if p1 >= threshold else (0 if target_class == 1 else 1)
        return {"pred": int(pred), "p1": p1, "threshold": threshold,
                "target_class": target_class, "classes": classes}

    # ---- (a) Diccionario simple: vectorizer + clf + threshold ----
    if isinstance(model_obj, dict) and "vectorizer" in model_obj and "clf" in model_obj:
        vec = model_obj["vectorizer"]
        clf = model_obj["clf"]
        threshold = float(model_obj.get("threshold", 0.5))
        classes = _get_classes_from_estimator(clf)
        X = vec.transform(Xtxt)
        pvec = _proba_or_score_to_p1(clf, X)

        # asumimos binario y que la "clase 1" es la positiva por defecto
        if classes is not None:
            if 1 in classes:
                idx = int(np.where(classes == 1)[0][0])
            else:
                idx = len(classes) - 1  # última posición como positiva si no está el "1"
        else:
            idx = 1
        ...
        return {"pred": int(pred), "p1": p1, "threshold": threshold,
                "target_class": 1, "classes": classes}

    # ---- (c) Estimador sklearn directo ----
    if hasattr(model_obj, "predict"):
        pred = int(model_obj.predict(Xtxt)[0])
        return {"pred": pred, "p1": float(pred), "threshold": 0.5,
                "target_class": 1, "classes": _get_classes_from_estimator(model_obj)}

    # Fallback
    return {"pred": 0, "p1": 0.0, "threshold": 0.5, "target_class": 1, "classes": None}


def evaluate_compliance_models(transcription_text: str):
    """
    Configura aquí cada atributo, el path del modelo y qué etiqueta significa 'Cumple'.
    MUY IMPORTANTE: 'cumple_label' DEBE coincidir con cómo entrenaste ese joblib.
    """
    model_cfg = [
        {
            "atributo": "Presenta alternativas de acuerdo con la necesidad",
            "path": "mejor_modelo_tfidf_alternativas.joblib",
            "cumple_label": 0,  # <-- AJUSTA si en tu entrenamiento '1' es 'Cumple'
        },
        {
            "atributo": "Condiciones de incumplimiento",
            "path": "mejor_modelo_RF_tfidf_ros06_condiciones.joblib",
            "cumple_label": 0,
        },
        {
            "atributo": "Explica medios de pago",
            "path": "modelo_cobranza_tfidf_explicacion.joblib",
            "cumple_label": 0,
        },
    ]
    ...
    for cfg in model_cfg:
        obj = load_model_safely(cfg["path"])
        if obj is None:
            rows.append({"Atributo": cfg["atributo"], "Cumple": "Modelo no disponible"})
            continue

        try:
            out = predict_label_from_model(obj, transcription_text)
            pred = int(out["pred"])
            cumple = "Cumple" if pred == int(cfg["cumple_label"]) else "No cumple"

            # (Opcional) Transparencia: muestra prob y umbral entre paréntesis
            p1 = out.get("p1", None)
            thr = out.get("threshold", None)
            if p1 is not None and thr is not None:
                cumple += f" (p1={p1:.2f} | thr={thr:.2f})"

            rows.append({"Atributo": cfg["atributo"], "Cumple": cumple})
        except Exception as e:
            rows.append({"Atributo": cfg["atributo"], "Cumple": f"Error: {e}"})
    return rows


# def display_compliance_table(transcription_text: str):
#     """Tabla compacta de Cumplimiento usando evaluate_compliance_models()."""
#     rows = evaluate_compliance_models(transcription_text)
#     df = pd.DataFrame(rows, columns=["Atributo", "Cumple"])
...
#         hide_index=True,
#         column_config={
#             "Atributo": st.column_config.TextColumn("Atributo", width="large"),
#             "Cumple": st.column_config.TextColumn("Cumple", width="medium"),
#         },
#     )
def display_compliance_table(transcription_text: str):
    """Tabla compacta de Cumplimiento con íconos ✅❌"""
    rows = evaluate_compliance_models(transcription_text)
    
    # Agregar los íconos a la columna Cumple
    for r in rows:
        if r["Cumple"].startswith("Cumple"):
            r["Cumple"] = "✅ " + r["Cumple"]
        elif r["Cumple"].startswith("No cumple"):
            r["Cumple"] = "❌ " + r["Cumple"]
...
def process_audio(uploaded_file, api_url: str, api_token: str, timeout: int, enable_transcription: bool = True, whisper_model: str = "base"):
    ...
    if enable_transcription:
        ...
        if transcription_result["success"]:
            ...
            # Mostrar tabla de cumplimiento basada en la transcripción
            display_compliance_table(transcription_result["transcription"])
        else:
            st.error(f"✕ Error en la transcripción: {transcription_result['error']}")
...
def main():
    ...
    with col1:
        ...
        # Tabla de IA de ejemplo removida; se mostrará la de cumplimiento tras transcribir
